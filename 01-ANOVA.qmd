...
Si vous ne savez plus à quoi correspondent les arguments `width` et `height` de `geom_jitter()`, prenez le temps de relire les explications de la section 3.5.2 du [livre en ligne de Biométrie 3](https://besibo.github.io/BiometrieS3/03-visualization.html#fig-jitter-1) consacrées à cette question. 
...
Le test de comparaisons multiples de Student est réalisé avec la fonction `pairwise.t.test()`. En réalité, ici, 3 tests de Student seront réalisés. Les $p-$values des tests seront simplement modifiées afin que globalement, le risque $\alpha$ n'augmente pas. Pour chaque test réalisé, les hypothèses nulles et alternatives sont les mêmes que celles décrites à la [section 3.6.1](https://besibo.github.io/BiometrieS5/03-TwoSampleTests.html#sec-robust) du livre en ligne du semestre 5 :
...
La particularité de l'ANOVA est sa grande **robustesse** vis-à-vis d'un non respect modéré de ses conditions d'application (voir définition de la robustesse dans la [section 3.6.1](https://besibo.github.io/BiometrieS5/03-TwoSampleTests.html#sec-robust) du livre en ligne du semestre 5). L'ANOVA étant particulièrement robuste, ses résultats resteront valides dans les situations suivantes :
